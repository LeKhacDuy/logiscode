...
export const getUsers = (req: Request, res: Response) => {
  ...
  const paginatedData = users.slice(startIndex, startIndex + limit).map(u => {
    ...
    return {
      ...userClean,
      password: u.rawPassword || '123456' // Trường mật khẩu hiển thị theo yêu cầu mới
    };
  });
...
export const createUser = (req: Request, res: Response) => {
  const { fullname, email, role, status = 'active' } = req.body;
  ...
  // Tự động sinh password 6 ký tự
  const autoGeneratedPassword = generateRandomPassword(6);
  ...
  const newUser: User = {
    ...
    passwordHash: bcrypt.hashSync(autoGeneratedPassword, 10),
  ...
  return res.status(201).json({
    success: true,
    message: `Thêm tài khoản mới thành công! Mật khẩu tự động tạo 6 ký tự: ${autoGeneratedPassword}`,
